...
NAS 上运行的 Docker Compose 服务编排仓库，通过 Dockge 可视化管理。

## 目录结构

```text
docker-stacks/
  compose.yml          # 根入口，仅启动 Dockge
  .env                 # 共用变量（唯一源文件）
  dockge/              # Dockge 可视化管理
...
# 4. 浏览器打开 http://<NAS_IP>:5001，导入 stacks/ 下的应用并启动
```
...
| 方式 | 命令 | 适用场景 |
|------|------|----------|
| Dockge UI | `http://<NAS_IP>:5001` | **主要方式**：可视化管理所有 compose 栈 |
