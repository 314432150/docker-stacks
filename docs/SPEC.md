...
- **输出**：stdout = JSONL（每行合法 JSON），stderr = 日志/警告
- **退出码**：0=成功, 1=参数错误, 2=锁冲突, 3=前置条件不满足
- **任务锁**：写入操作（backup/restore/deploy）受 `$ROOT/.cache/engine.lock` 保护
  ...
  "apps": [
    {
      ...
      "dirs": [
        {"path": "stacks/qbittorrent/data", "recommended": true, "exists": true},
        {"path": "stacks/qbittorrent/config", "recommended": false, "exists": true}
      ]
    },
    {
      "name": "openclaw",
      "description": "",
      "dirs": [
        {"path": "stacks/openclaw/data", "recommended": true, "exists": true}
      ]
    }
  ]
}
